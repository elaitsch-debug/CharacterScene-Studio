...
export const CharacterCreatorModal: React.FC<CharacterCreatorModalProps> = ({ onClose, onSave }) => {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const img = await generateCharacterImage(description);
  ...
  const renderContent = () => {
    ...
    switch (step) {
      ...
      case 'DESCRIBE':
        return (
          <div>
            <h3 className="text-lg font-semibold mb-2">Refine Description</h3>
            <div className="flex gap-4">
              <img src={imageBase64} alt="Uploaded character" className="w-1/3 h-auto object-contain rounded-md" />
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                className="w-2/3 h-64 p-2 bg-gray-800 border border-gray-600 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div className="flex gap-4">
              ...
              <div className="w-2/3 flex flex-col gap-4">
                ...
                <textarea
                  value={description}
                  readOnly
                  className="w-full h-48 p-2 bg-gray-800 border border-gray-600 rounded-md text-gray-400"
                />
              </div>
            </div>
            <div className="mt-4 flex justify-end">
